...
- `img/g-*.jpg` — the gallery photos, listed in `data.json` → `galeria`. Optimised historical scans (~1400px, progressive JPEG). Two of them (`g-1972-*`, `g-1974-*`) are reused as the `img` of the matching `trayectoria` milestones.
- `img/t*.svg` — remaining placeholder stand-ins for `trayectoria` milestones without a real photo yet (`t1`, `t3`, `t4`, `t5`). Keep the filename or update `data.json`.
